...
复现成功后，`engineering-repro` 自动生成 `FROZEN_SPEC.md`，**锁死两项内容**：

| 锁定项 | 策略 | 违反时 |
|--------|------|--------|
| **数据准备**（数据集、预处理、划分） | 完全冻结 | 拒绝执行，提示用户 |
| **评估标准**（指标、评估脚本、测试集） | 完全冻结 | 拒绝执行，提示用户 |
| **数据增强**（加噪、混响、变速等） | 需用户批准 | 询问用户；若批准则 baseline 也必须用相同增强重跑 |

改进阶段只能修改：特征提取、模型架构、训练算法、超参数、损失函数。数据和评估不动，确保对比公平。
